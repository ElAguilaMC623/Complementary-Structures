...
public class JigsawGenerationChecker extends Structure {

    public static final MapCodec<JigsawGenerationChecker> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            settingsCodec(instance),
            StructureTemplatePool.CODEC.fieldOf("start_pool")
                    .forGetter(s -> s.startPool),
            Codec.intRange(0, 30).fieldOf("size")
                    .forGetter(s -> s.size),
            Codec.BOOL.fieldOf("cannot_spawn_in_liquid")
                    .orElse(false).forGetter(s -> s.cannotSpawnInLiquid),
            Codec.intRange(1, 100).optionalFieldOf("terrain_height_check_radius")
                    .forGetter(s -> s.terrainHeightCheckRadius),
            Codec.intRange(1, 1000).optionalFieldOf("allowed_terrain_height_range")
                    .forGetter(s -> s.allowedTerrainHeightRange)
    ).apply(instance, JigsawGenerationChecker::new));
    ...
    public JigsawGenerationChecker(StructureSettings settings,
    ...
    protected boolean spawningChecks(GenerationContext context, BlockPos blockPos) {
        ...
        if (this.cannotSpawnInLiquid) {
            ...
            int landHeight = context.chunkGenerator().getFirstOccupiedHeight(center.getX(),
                    center.getZ(), Heightmap.Types.WORLD_SURFACE_WG,
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        return ComplementJigsawAssembler.assemble(
                ...
                this.size,
                pos,
                CSStructureGeneration.JIGSAW_STRUCTURE_GENERATION.get()
        );
    }

    @Override
    public StructureType<?> type() {
        return CSStructureGeneration.JIGSAW_STRUCTURE_GENERATION.get();
    }
}